...
학교 행사와 교사 연수의 참가 신청, 담당 교사별 등록부 수합, 참가자 관리, 엑셀 다운로드, 인쇄용 등록부 생성을 처리하는 React 웹앱입니다.
...
VITE_PUBLIC_APP_URL=https://sign-vert-seven.vercel.app
...
- 행사별 등록부 인쇄: `/event/{행사ID}/attendance`

공개 등록 페이지는 행사 정보와 본인 등록 폼만 표시합니다. 참가자 전체 명단 조회, 엑셀 다운로드, 등록부 인쇄는 관리자 또는 해당 행사 담당 교사 로그인 후 접근하도록 분리했습니다.
...
- 관리자 또는 해당 담당 교사 권한 확인 후 행사 수정, 삭제, 참가자 관리, 등록부 인쇄
- 공개 등록 링크 복사와 QR 코드 이미지 제공
- 행사 등록 시 정원 수에 맞춘 사전 명단 직접 입력
- Excel 또는 CSV 사전 명단 업로드와 미리보기
- 공개 등록 입력 항목: 소속, 직위, 성명, 서명
- 관리자 사전 참가자 등록과 일괄 업로드
- 중복 참가자 확인
- 참가자 검색, 필터, 수정, 삭제
- 참석/미참석 처리와 직접 서명, 이미지 서명 업로드 관리
- 전체 참가자 엑셀 다운로드
- 서명이 포함된 공개 참가자 등록 폼과 사전 등록자 서명 폼
- A4 인쇄용 등록부와 `@media print` 전용 CSS
- 시연용 데이터 생성 버튼
